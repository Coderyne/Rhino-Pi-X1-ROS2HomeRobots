function K = get_k_length(leg_length)
    ...
    l1=0.07104;                       %机体质心距离转轴距离
    mw1=1.21022;                    %两轮总质量(2*0.60511kg)
    mp1=3.88658;                    %两腿总质量(2*1.94329kg)
    M1=2.08743;                     %机体总质量
    Iw1=0.0019156389;               %两轮绕轮轴总转动惯量
    Ip1=0.05306578886;              %两腿绕质心总转动惯量
    IM1=0.01533588908;              %机体绕质心转动惯量
...
%         R=[240 0;0 25];
    ...
    Q=diag([1 0.15 8 16 450 1.2]); % 提高速度/姿态权重，减小超调
   R=[20 0;0,1];                %T Tp
