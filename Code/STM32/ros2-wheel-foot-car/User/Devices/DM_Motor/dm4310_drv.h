...
typedef enum {
	WHEEL_STATE_ENABLED = 0x0,
	WHEEL_STATE_DISABLED = 0x1,
	WHEEL_STATE_OVER_VOLTAGE = 0x8,
	WHEEL_STATE_UNDER_VOLTAGE = 0x9,
	WHEEL_STATE_OVER_CURRENT = 0xA,
	WHEEL_STATE_OVER_MOS_HEAT = 0xB,
	WHEEL_STATE_OVER_MOTOR_HEAT = 0xC,
	WHEEL_STATE_LOST_COMMUNICATION = 0xD,
	WHEEL_STATE_OVERLOAD = 0xE
} WHEEL_MOTOR_STATE;


extern void dm4310_fbdata(Joint_Motor_t *motor, uint8_t *rx_data,uint32_t data_len);
extern void dm6215_fbdata(uint8_t *rx_data,uint32_t data_len);
...
extern void mit_6215(hcan_t* hcan,uint16_t motor_id,float torq);
