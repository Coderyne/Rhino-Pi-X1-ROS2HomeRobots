...
void dm6215_fbdata(uint8_t *rx_data,uint32_t data_len)
{
	uint8_t motorID = rx_data[0] & 0x0F;
	WHEEL_MOTOR_STATE state = rx_data[0] >> 4;
	...
	{//返回的数据有8个字节
	  ...
	  {
	  	case 0x05:
			dm6215_update_data(&chassis_move.wheel_motor[0], rx_data, data_len);break; // 更新右边轮毂电机数据

		case 0x06:
			dm6215_update_data(&chassis_move.wheel_motor[1], rx_data, data_len);break; // 更新左边轮毂电机数据
...
void dm6215_update_data(Wheel_Motor_t *motor, uint8_t *rx_data,uint32_t data_len)
{
	motor->para.id = (rx_data[0])&0x0F;
	motor->para.state = (rx_data[0])>>4;
	motor->para.p_int=(rx_data[1]<<8)|rx_data[2];
...
void mit_6215(hcan_t* hcan,uint16_t motor_id,float torq)
{
	uint8_t data[8];
	uint16_t tor_tmp;
	uint16_t id = motor_id + MIT_MODE;
